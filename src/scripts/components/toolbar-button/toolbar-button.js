...
export default class ToolbarButton {
  ...
  constructor(params = {}, callbacks = {}) {
    ...
    this.params.props?.forEach((prop) => {
      this.button.setAttribute(Object.keys(prop)[0], prop[Object.keys(prop)[0]]);
    });
    ...
    if (this.button.draggable) {
      ...
      this.button.addEventListener('dragstart', (event) => {
        this.params.dataTransferPairs.forEach((pair) => {
          event.dataTransfer.setData(pair.type, pair.data);
        });
    ...
    this.button.addEventListener('blur', () => {
      if (this.params.type === 'toggle' && this.active) {
        this.toggle();
      }
    });
