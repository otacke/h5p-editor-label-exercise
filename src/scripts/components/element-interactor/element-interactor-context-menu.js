...
import './element-interactor-context-menu.scss';

export default class ElementInteractorContextMenu {
   ...
   * @param {function} [callbacks.getDenominator] Get denominator.
   */
  constructor(params = {}, callbacks = {}) {
    ...
    this.dom.classList.add('h5peditor-label-exercise-element-interactor-context-menu');
   ...
   * @returns {object|null} Button or null if not available.
   */
  getButton(id = '') {
    if (!this.buttons[id]) {
      return null; // Button not available
    }
   ...
   */
  updateAriaLabel() {
    const denominator = this.callbacks.getDenominator();
    const ariaLabel = this.params.dictionary.get('a11y.contextMenu').replace('@denominator', denominator);
