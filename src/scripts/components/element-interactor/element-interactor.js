import { extend, callOnceVisible, multiClick } from '@services/util.js';
...
/** @constant {number} TELEMETRY_DEFAULT_WIDTH_PX Default width of interactor element in pixels */
const TELEMETRY_DEFAULT_HEIGHT_PX = 30;

/** @constant {number} TELEMETRY_MIN_HEIGHT_PX Minimum height of interactor element in pixels */
const TELEMETRY_MIN_HEIGHT_PX = Math.max(TELEMETRY_DEFAULT_HEIGHT_PX, 1);

/** @constant {number} TELEMETRY_MIN_WIDTH_PX Minimum width of interactor element in pixels */
const TELEMETRY_MIN_WIDTH_PX = 32;
...
export default class ElementInteractor {
  ...
  constructor(params = {}, callbacks = {}) {
    ...
    this.params = extend({
      telemetry: {
        ...
        height: this.pxToPercent(TELEMETRY_DEFAULT_HEIGHT_PX, 'height'),
      },
      capabilities: {
        edit: true,
        move: true,
  ...
  /**
   * Match content height to rendered content.
   */
  matchContentHeight() {
    const contentHeight = this.contentDOM.getBoundingClientRect().height;
    this.setTelemetry({ height: this.pxToPercent(contentHeight, 'height') });
  }
  ...
  buildDOM() {
    ...
    this.dom.classList.add('h5peditor-label-exercise-element-interactor');
    ...
    this.ariaSummary.classList.add('h5peditor-label-exercise-element-interactor-summary');
    ...
    if (this.params.capabilities.resizeX || this.params.capabilities.resizeY) {
  ...
  buildContextMenu() {
    ...
    if (this.params.capabilities.bringToFront) {
      contextMenuButtonParams.push({
        id: 'bringToFront',
        type: 'pulse',
    ...
    if (this.params.capabilities.sendToBack) {
      contextMenuButtonParams.push({
        id: 'sendToBack',
        type: 'pulse',
  ...
  buildResizeKnobs() {
    ...
    if (this.params.capabilities.resizeX) {
      knobPositions.push('right', 'left');
    }
    if (this.params.capabilities.resizeY) {
      knobPositions.push('top', 'bottom');
    }
    if (this.params.capabilities.resizeX && this.params.capabilities.resizeY) {
      knobPositions.push('top-left', 'top-right', 'bottom-left', 'bottom-right');
    }

    knobPositions.forEach((position) => {
  ...
  setTelemetry(telemetry = {}, options = {}) {
    ...
    this.dom.style.setProperty('--editor-label-exercise-element-x', `${this.telemetry.x}%`);
    this.dom.style.setProperty('--editor-label-exercise-element-y', `${this.telemetry.y}%`);
    this.dom.style.setProperty('--editor-label-exercise-element-width', `${this.telemetry.width}%`);
    this.dom.style.setProperty('--editor-label-exercise-element-height', `${this.telemetry.height}%`);
  }
  ...
  isSizeWithinBounds(size = {}) {
    ...
    if (typeof width === 'number' && (width < this.pxToPercent(TELEMETRY_MIN_WIDTH_PX, 'width') || width > 100)) {
      return false;
    }

    if (typeof height === 'number' && (height < this.pxToPercent(TELEMETRY_MIN_HEIGHT_PX, 'height') || height > 100)) {
      return false;
    }
  ...
  getSanitizedSize(size = {}) {
    return {
      width: Math.max(
        this.pxToPercent(TELEMETRY_MIN_WIDTH_PX, 'width'),
        Math.min(size.width ?? this.telemetry.width, 100),
      ),
      height: Math.max(
        this.pxToPercent(TELEMETRY_MIN_HEIGHT_PX, 'height'),
  ...
  updateTelemetryByPx(deltaPx = {}, options = {}) {
    ...
    if (Math.ceil(targetTelemetry.width) < TELEMETRY_MIN_WIDTH_PX) {
      return;
    }

    if (Math.ceil(targetTelemetry.height) < TELEMETRY_MIN_HEIGHT_PX) {
      return;
    }
  ...
  handleMoveEnd(event, options) {
    ...
    this.Arrow = 1;
    this.callbacks.onMove(this.params.id);
  }
  ...
  handleFocusOut(event) {
    ...
    if (
      ...
      event.relatedTarget?.closest('.h5peditor-label-exercise-element-interactor') === this.dom
    ) {
